...
- **Unix-style flags** — `wc -l`, `grep -v`, `head -n 10`, `cut -d , -f 2`, the GNU `head -5`/`tail -3` shorthand — translated to each command's typed options.
- **Shell expansion** — globbing (`*`, `?`, `[…]`) and `~` expand against the filesystem; quotes suppress expansion; non-matching globs stay literal.
- **File or stdin input** — file arguments on the first command are opened as the pipeline's input; otherwise input is read from stdin.

...
- **Generate**: `echo`, `emit`, `seq`, `yes` _(sources)_
- **Filesystem**: `ls`, `find` _(sources)_, `basename`, `dirname`
- **Search & filter**: `grep`, `comm`, `uniq`
- **Transform**: `tr`, `sed`, `perl`, `rev`, `cut`, `paste`, `split`, `join`, `nl`, `cat`, `tac`, `tail`, `head`
...
- `awk` — takes a `Program` **interface** (Begin/Condition/Action/End); there is no string→`Program` parser.
